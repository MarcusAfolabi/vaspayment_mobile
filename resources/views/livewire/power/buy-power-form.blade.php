<div>
    <div class="modal-body">
        ...
        <div class="quick-send mb-2 border-b-2">
            ...
            <div class="profile" wire:click="$set('phone', '{{ $item['phone'] }}'); $set('network', '{{ $item['network'] }}')">
                <div @click.prevent="$wire.set('phone', '{{ $item['phone'] }}'); $wire.set('network', '{{ $item['network'] }}')" class="d-flex justify-content-between align-items-center">
                    <img src="{{ asset('assets/images/networks/' . strtolower($item['network']) . '.jpg') }}" class="img-fluid" style="height: 30px; width: 30px;" alt="{{ $item['network'] }}">
        ...
        <form class="auth-form p-0 mt-2" style="border-top: 2px solid #650A88;">
            ...
            <div class="form-group">
                <div class="row">
                    ...
                    <div class="d-flex justify-content-between text-center gap-2 mb-2" style="flex-wrap: wrap;">
                        ...
                        $networkLogo = 'assets/images/networks/IKEJA.jpg';
                        break;

                        case $plan['provider'] === 'JEDC':
                        $networkLogo = 'assets/images/networks/JOS.jpg';
                        break;

                        case $plan['provider'] === 'KAEDC':
                        $networkLogo = 'assets/images/networks/KADUNA.jpg';
                        break;

                        case $plan['provider'] === 'KEDC':
                        $networkLogo = 'assets/images/networks/KANO.jpg';
                        break;

                        case $plan['provider'] === 'PHEDC':
                        $networkLogo = 'assets/images/networks/PHO.jpg';
                        break;
                        ...
                        <div class="network-item" style="flex: 1 0 calc(20% - 10px);"> <!-- Each item takes up 20% width, with small margin -->
                            <img src="{{ asset($networkLogo) }}" style="height: 25px; width: 25px;" alt="{{ $plan['provider'] }}" class="network-img rounded {{ ($selectedNetwork === $plan['provider'] || $network === $plan['provider']) ? 'selected' : '' }}" wire:click="$set('selectedNetwork', '{{ $plan['provider'] }}')" />
            ...
            <div class="form-group">
                <label for="inputamount1" class="form-label">Meter No</label>
